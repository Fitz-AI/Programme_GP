...
{
BaseWitt(A)=
local(n,P,u,Z,X,Y,M,tmp1,p,tmp2,B);
n=#A;
P=matid(n);
...
{
qf13(A,B)=
local(n,tmp,a,y,R,A1,B1,z1,z2,P,P2,A2,B2,z3,r,z,s,u,v,P3,P4,A3,A4,S2,E,w,x);
n=#A;
tmp=qfsign(A);
...
z=Final3(A2,B2,z3,r);
s=sign(eval(z*B2*z~));
while(s==1, r=2*r; z=Final3(A2,B2,z3,r); s=sign(eval(z*B2*z~)));
z=z*P;
\\z=z/content(z);
while( y*A*z~==0, 
                 u=vector(n,i,random(50)-20); 
...
A3=P4*A*P4~;
B3=P4*B*P4~;
C=vecextract(A3,Str(3".."n),Str(3".."n));
D=vecextract(B3,Str(3".."n),Str(3".."n));
C=C/content(C);
S=Hyperbolique(C);
P5=matid(n);
for(i=3,n,P5[i,]=S[i-2,]);
A4=P5*A3*P5~;
B4=P5*B3*P5~;
P=P5*P4;
if ( B4[3,3]>0, 
...
E=matrix(5,5);
for(i=1,4,
          for(j=1,4,E[i,j]=B4[2*i-1,2*j-1])
   );
for(i=1,4,E[5,i]=B4[9,2*i-1]);
for(i=1,4,E[i,5]=B4[2*i-1,9]);
E[5,5]=B4[9,9];
w=myqfsolve(E);
x=vector(n);
